...
import { productsPerLoad } from '../products/Products';

const Loading = () => {
    const loadingProducts = Array(productsPerLoad).fill("");
    return (
        <div className='cards_wrapper mb-[24px] lg:mb-[35px] grid grid-cols-1 min-[320px]:grid-cols-2 gap-x-[5px] gap-y-[8px] sm:gap-y-[12px] lg:gap-x-[10px] lg:gap-y-[16px] min-[700px]:gap-[7px] min-[480px]:grid-cols-3 min-[940px]:grid-cols-4 lg:grid-cols-5'>
            {
                loadingProducts.map((_, index) => (
                    <div key={index} className='card border border-[hsl(214,12%,27%)] rounded-[8px] shadow-xs overflow-hidden'>
